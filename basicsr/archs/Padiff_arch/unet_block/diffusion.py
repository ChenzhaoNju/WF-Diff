...
class GaussianDiffusion(nn.Module):
    ...
    def set_loss(self, device):
        ...
        self.loss_func_ph2 = nn.L1Loss().to(device)
        self.loss_func_nrn = nn.L1Loss().to(device)
    ...
    def p_mean_variance_ddim(self, x, t,p_a,p_t, clip_denoised: bool, condition_x=None, style=None):
        if condition_x is not None:
            # x_recon = self.predict_start_from_noise(
            #     x, t=t, noise=self.denoise_fn(torch.cat([condition_x, x], dim=1), t, style))
            x_recon = self.denoise_fn(torch.cat([condition_x, x], dim=1), t,p_a,p_t)
        else:
            # x_recon = self.predict_start_from_noise(
            #     x, t=t, noise=self.denoise_fn(x, t))
            x_recon = self.denoise_fn(x, t,p_a,p_t)
        # if clip_denoised:
        #     x_recon.clamp_(-1., 1.)
    ...
    @torch.no_grad()
    def p_sample_ddim(self, x, t, t_next, p_a,p_t,clip_denoised=True, repeat_noise=False, condition_x=None, style=None):
        b, *_, device = *x.shape, x.device
        bt = extract(self.betas, t, x.shape)
        at = extract((1.0 - self.betas).cumprod(dim=0), t, x.shape)

        if condition_x is not None:
            et = self.denoise_fn(torch.cat([condition_x, x], dim=1), t,p_a,p_t)
        else:
            et = self.denoise_fn(x, t,p_a,p_t)
    ...
    def p_sample_ddim2(self, x, t, t_next, p_t, clip_denoised=True, repeat_noise=False, condition_x=None, style=None):
        b, *_, device = *x.shape, x.device
        bt = extract(self.betas, t, x.shape)
        at = extract((1.0 - self.betas).cumprod(dim=0), t, x.shape)

        if condition_x is not None:
            et = self.denoise_fn(torch.cat([condition_x, x], dim=1), t,p_t)
        else:
    ...
    def p_sample_loop(self, x_in, continous=False, cand=None):
        ...
        if not self.conditional:
            x = x_in['SR']
            # _,p_a ,p_t = self.physical_fn(x_in['SR'],x_in['HR'])

            p_a ,p_t = self.physical_fn(x_in['SR'])
            # condition_x = p_a
            # generate_gt = self.get_gener_out(x_in['SR'],p_t,p_a).detach()
            nrn_out = self.nrn_fn(generate_gt)
            ...
            for j, i in enumerate(tqdm(time_steps, desc='sampling loop time step', total=len(time_steps))):
                ...
                img = self.p_sample_ddim2(img, t, t_next, p_t, style=x_in['style'])
                if i % sample_inter == 0:
            ...
            condition_x = torch.concat([p_a,generate_gt+nrn_out],dim=1)
            # x = p_a
            ...
            if self.sample_proc == 'ddpm':
                ...
                for j, i in enumerate(time_steps):
                    ...
                    if j == len(time_steps) - 1:
                        ...
                        t_next = torch.full((b,), time_steps[j + 1], device=device, dtype=torch.long)
                    img = self.p_sample_ddim2(img, t, t_next, p_t, condition_x=condition_x)
                    if i % sample_inter == 0:
                        ret_img = torch.cat([ret_img, img], dim=0)
        if continous:
            return ret_img
    ...
    @torch.no_grad()
    def get_gener_out(self,img_input,out_t, out_a):
        return img_input / out_t + out_a * (1 - 1 / out_t)

    def p_losses(self, x_in, noise=None):
        x_start = x_in['HR']
        # condition_x = x_in['SR']
        p_a ,p_t = self.physical_fn(x_in['SR'])
        phy_out = x_in['HR'] * p_t + (1 - p_t) * p_a
        ...
        if not self.conditional:
            ...
            x_recon = self.denoise_fn(
                torch.cat([condition_x, x_noisy], dim=1), t, p_t)


        loss = self.loss_func(noise, x_recon)
        loss2 = self.loss_func_ph2(phy_out, x_in['SR'])
        loss3 = self.loss_func_nrn(nrn_out, x_in['HR'])
